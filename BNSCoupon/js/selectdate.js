//功能按钮同样样式
var s_tiannet_turn_base = "height:16px;font-size:10pt;color:white;border:0 solid #CCCCCC;cursor:hand;background-color:#2650A6;";
//翻年、月等的按钮
var s_tiannet_turn = s_tiannet_turn_base;
//关闭、清空等按钮样式
...
//显示日的td的样式
var s_tiannet_day = "width:22px;height:21px;background-color:#D8F0FC;font-size:11pt;";
//字体样式
...
for (var i = 1; i <= 12; i++) {
    if (i < 10)
        document.writeln('<option value="' + i + '">' + "0" + i + '月</option>');
    else
...
document.write('<a style="color:white;padding-right:4px;cursor: pointer;text-decoration:none;" onclick="tiannetHideControl();" title="关闭">×</a>')
//document.write('</div>');
...
//输出一条横线
document.write('<div style="'+s_tiannet_line+'"></div>');
document.write('<table cellspacing=0 cellpadding=0  bgcolor=white onselectstart="return false">');
document.write(' <tr style="background-color:#2650A6;font-size:10pt;color:white;height:22px;" Author="tiannet">');
for(var i =0;i < weekName.length;i ++){
    //输出星期
    document.write('<td width="22px" align="center" Author="tiannet">' + weekName[i] + '</td>');
}
document.write(' </tr>');
...
document.write (' </tr>');
document.write('</table>');
document.write('<div style="background-color:#2650A6; height:1px;"></div>');
document.write('</div>');
//------------------ end 页面显示部分 ---------------------------//
//------------------ 显示日期时间的span标签响应事件 ---------------------------//
//单击年份span标签响应
...
function tiannetWriteHead(){
    ...
    if (tiannetMonth < 10 && tiannetMonth.toString().length < 2)
        document.all.tiannetMonthHead.innerText = "0" + tiannetMonth + "月";
    else
...
//弹出显示日历选择控件，以让用户选择
function tiannetPopCalendar(obj){
    //隐藏下拉框，显示相对应的head
    ...
    if (iX <0)  
        iX=0;
    //计算top
    iY = getTop(obj) + obj.offsetHeight;
    //iY = window.event.y;
