...
namespace BNSCoupon
{
    public partial class PointLog : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                hlkStore.NavigateUrl = "StorePoint.aspx?id=" + Request.QueryString["id"];
            }
        }
        ...
        public void getAccount(CouponBLL bll, int id)
        {
            ...
            labTitle.Text = litTitle.Text = "积分购买记录-" + row["name"];
        }
        ...
        public DataSet getOrderList(CouponBLL bll, int id)
        {
            ...
            {
                case "0"://本月
                    ds = bll.getOrderPointThisMonth(id);
                    break;
                case "1"://上月
                    ds = bll.getOrderPointLastMonth(id);
                    break;
                case "2"://近三个月
                    ds = bll.getOrderPointThreeMonth(id);
                    break;
                case "3"://三个月前
                    ds = bll.getOrderPointLastThreeMonth(id);
                    break;
                case "4"://全部记录
                    ds = bll.getOrderPointAll(id);
                    break;
                default:
                    ds = bll.getOrderPointThisMonth(id);
                    break;
